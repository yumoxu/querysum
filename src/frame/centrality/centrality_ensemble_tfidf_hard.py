...
from utils.tools import get_test_cc_ids, get_text_dp_for_tdqfs

import tools.vec_tools as vec_tools
import summ.select_sent as select_sent
import summ.compute_rouge as rouge

import frame.centrality.centrality_ensemble_config as centrality_ensemble_config
...
"""
    This file is for the combination of the following components:
        1. QA model: Bert-Ensemble; score and filter
        2. Centrality model: MRW (sentence rep: TFIDF); bias: QA
...
"""
qa_record_dp = join(path_parser.summary_rank, centrality_ensemble_config.QA_RECORD_DIR_NAME)

use_tdqfs = 'tdqfs' in qa_record_dp
if use_tdqfs:
    ...
    test_cid_query_dicts = general_tools.build_test_cid_query_dicts(tokenize_narr=False,
                                                                    ...
                                                                    query_type=centrality_ensemble_config.QUERY_TYPE)
...
def _build_components(cid, query):
    ...
    rel_vec = vec_tools.norm_rel_scores(rel_scores=rel_scores, max_min_scale=False, passage_proc=False)  # score has been scaled
...
def score_e2e():
    damp = centrality_ensemble_config.DAMP
    use_rel_vec = True
    if damp == 1.0:
        damp = 0.85
        use_rel_vec = False

    graph_tools.score_end2end(model_name=centrality_ensemble_config.CENTRALITY_MODEL_NAME_BASIC,
                              damp=damp,
...
def rank_e2e():
    graph_tools.rank_end2end(model_name=centrality_ensemble_config.CENTRALITY_MODEL_NAME_BASIC,
                             diversity_param_tuple=centrality_ensemble_config.DIVERSITY_PARAM_TUPLE,
                             retrieved_dp=qa_record_dp,
                             cc_ids=cc_ids)
...
def select_e2e():
    params = {
        ...
        'n_iter': None,
        'diversity_param_tuple': centrality_ensemble_config.DIVERSITY_PARAM_TUPLE,
        'cos_threshold': centrality_ensemble_config.COS_THRESHOLD,  # do not pos cosine similarity criterion?
        'retrieved_dp': qa_record_dp,
    }
...
def select_e2e_tdqfs():
    params = {
        'model_name': centrality_ensemble_config.CENTRALITY_MODEL_NAME_BASIC,
        'n_iter': None,
        'length_budget_tuple': centrality_ensemble_config.LENGTH_BUDGET_TUPLE,
        'diversity_param_tuple': centrality_ensemble_config.DIVERSITY_PARAM_TUPLE,
        'cos_threshold': centrality_ensemble_config.COS_THRESHOLD,  # do not pos cosine similarity criterion?
        'retrieved_dp': qa_record_dp,
        'cc_ids': cc_ids,
...
def compute_rouge_tdqfs():
    text_params = {
        ...
        'length_budget_tuple': centrality_ensemble_config.LENGTH_BUDGET_TUPLE,
        'diversity_param_tuple': centrality_ensemble_config.DIVERSITY_PARAM_TUPLE,
        'cos_threshold': centrality_ensemble_config.COS_THRESHOLD,
        'extra': None,
    }
    ...
    if centrality_ensemble_config.LENGTH_BUDGET_TUPLE[0] == 'nw':
        rouge_parmas['length'] = centrality_ensemble_config.LENGTH_BUDGET_TUPLE[1]
...
def tune():
    ...
    :return:
    """
    diversity_algorithm = centrality_ensemble_config.DIVERSITY_ALGORITHM

    if diversity_algorithm == 'wan':
        tune_range = range(0, 16, 1)
    else:
        raise ValueError('Invalid diversity_algorithm: {}'.format(diversity_algorithm))

    centrality_tune_dp_rank = join(path_parser.summary_rank, centrality_ensemble_config.CENTRALITY_TUNE_DIR_NAME_BASIC)
    centrality_tune_dp_text = join(path_parser.summary_text, centrality_ensemble_config.CENTRALITY_TUNE_DIR_NAME_BASIC)

    centrality_tune_result_fp = join(path_parser.tune, centrality_ensemble_config.CENTRALITY_TUNE_DIR_NAME_BASIC)
