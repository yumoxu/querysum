...
from frame.bert_qa.bert_input_cosine import build_query, build_sentence


class ClusterDataset(Dataset):
    def __init__(self, cid, retrieve_dp, transform=None):
    ...
    def __getitem__(self, index):
        ...
        """
        # build xx
        sent = build_sentence(sentence=self.sentences[index])
...
class ClusterDataLoader(DataLoader):
    def __init__(self, cid, query, retrieve_dp, transform=data_tools.ToTensor()):
        ...
        query_dict = build_query(query)
        for (k, v) in query_dict.items():
            v = v.reshape(1, -1)
            query_dict[k] = torch.from_numpy(v)
        self.query_in = self.transform(query_dict)
